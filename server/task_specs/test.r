...
get_data_file <-function(files, mtypes, data_type) {
  for (fid in 1:length(files)){
    if (grepl(data_type,mtypes[fid])){
      return(files[fid])
    }
  }
   return("error")
...
files = c(input_path_1, input_path_2, input_path_3)
mtypes = c(metadata_1, metadata_2, metadata_3)

mRNA.m <- read.csv(get_data_file(files, mtypes,"Trans"))
output_mrna_dim = dim(mRNA.m)

miRNA.m <- read.csv(get_data_file(files, mtypes, "micro"))
output_mirna_dim = dim(miRNA.m)

clinical.m <- read.csv(get_data_file(files, mtypes, "Clinical"))
